...
The check whether one actor from one movie is also in the other movie, we do not use strict matching, but rather a fuzzy approach.
Two actors are considered equal (but only for the sake of matching movies), when their Levenshtein\footnote{The Levenshtein distance between two strings is defined as the amount of edit operations (insertions, deletions, substitutions), which are needed to transform the first string into the second one.} distance is below a certain threshold $ACT\_DIST$.
