...
The results of an analysis of wrong matches came to the following conclusions:
In general, if the systems returns a match, this is very likely to be correct (as indicated by the high precision).
In the cases, where an incorrect match was returned, this was most likely a match for a movie, which was not in the database, i.e. the system thought it found a match from the database, but actually, there was not.
This was the case in 85\% of the returned matches.
...
\begin{itemize}
	...
	\item
    In the remaining cases, the threshold was to low.
\end{itemize}
