...
With the determined parameters $MIN\_SCORE = 0.2$, $ACT\_DIST = 2$ and $CANDIDATE\_SET = 32$ the evaluation ran on another test set as described above.
...
The baseline approach matches, if and only if the movie names (considering both the movie name and the original title) match exactly.
