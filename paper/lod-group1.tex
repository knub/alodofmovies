...
The world wide web contains lots of pages about movies, but most of them offer only unstructured content.
This paper describes the system \emph{Match Forrest, Match!}, which aggregates and links movie data from different online movie data sources to a unified, linked open dataset.
For that, the IMDb, TMDb, OFDb and Freebase are used.
...
The algorithm is evaluated and the results show a high precision, while still maintaining a good recall.
Besides, the system updates all movies periodically, so that the resulting dataset is always up-to-date.
...
%\clearpage
%\newpage
\input{chapters/3_method/3_method}
%\clearpage
%\newpage
\input{chapters/4_evaluation/4_evaluation}
%\newpage
%\clearpage
\input{chapters/5_discussion}
%\clearpage
%\newpage
\input{chapters/6_conclusion_and_outlook}
