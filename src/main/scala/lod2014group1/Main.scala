...
object Main extends App with Logging {
	...
	override def main(args: Array[String]): Unit = {
		...
		if (args contains "triplify") {
			...
			val crawlTask1 = WorkerTask(TaskType.Crawl.toString, Map("uri" -> "http://Kung_Fu_Panda", "task_id" -> "1"))
			val triplifyTask1 = WorkerTask(TaskType.Triplify.toString, Map("uri" -> "Kung_Fu_Panda", "task_id" -> "2"))
			val crawlTask2 = WorkerTask(TaskType.Crawl.toString, Map("uri" -> "http://Fight_Club", "task_id" -> "3"))
			val triplifyTask2 = WorkerTask(TaskType.Triplify.toString, Map("uri" -> "Fight_Club", "task_id" -> "4"))
			val crawlTask3 = WorkerTask(TaskType.Crawl.toString, Map("uri" -> "http://Godzilla", "task_id" -> "5"))
			val triplifyTask3 = WorkerTask(TaskType.Triplify.toString, Map("uri" -> "Godzilla", "task_id" -> "6"))
			val crawlTask4 = WorkerTask(TaskType.Crawl.toString, Map("uri" -> "http://August_Rush", "task_id" -> "7"))
